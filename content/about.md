...
- **Stride** — an SMS productivity coach. [Strands SDK](https://strandsagents.com) + Claude Sonnet 4.6, 19 tools, Haiku intent classifier for cost control, eval suite in CI with LLM-as-judge rubrics, single-table DynamoDB (14 entity types), full A2P 10DLC compliance.
- **Apex** — a proactive Telegram health accountability bot. Bedrock + Strands, protocol YAML drives dynamic tool generation at cold start — add a metric to config and the agent gains a tool, zero code changes. 9 EventBridge schedules drive check-ins throughout the day.
